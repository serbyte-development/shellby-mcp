...
`test:live:fixture` never submits a prompt. Its package script loads `.env` when present, then uses the production background-page creator and saved-conversation navigation path, captures the current `/backend-api/conversations/<id>` payload, and validates the production parser, exact `content.parts` Markdown, conversation route, and rendered DOM against `test/fixtures/chatgpt-live-fixture/conversation.json` and `test/fixtures/chatgpt-project-live-fixture/conversation.json`. The project fixture runs only when `MCP_CHATGPT_PROJECT_URL` is set and is otherwise skipped. `test:live:subagent` is a black-box MCP canary: it starts the normal MCP server, uses only public `subagent_run`/`subagent_result` calls, and reuses one `agent_id` for two sequential generated turns. It does not inspect module internals, tracker state, DOM, Page objects, or conversation IDs. Turn 2 proves persistent context by recalling a random key supplied only in Turn 1. Sanitized evidence from the most recent generative run is written under ignored `test/live/artifacts/`.
