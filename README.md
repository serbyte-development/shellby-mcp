...
The shared instructions, `shell_run` description, and command schema tell the model to prefer installed RTK equivalents for supported commands. Search should start with `rtk rg` or `rtk rg --files`; other examples include `rtk test npm test` instead of `npm test` and `rtk git diff` instead of `git diff`. RTK is guidance only: the server does not rewrite commands, and raw shell commands remain available when exact unfiltered output, persistent state changes, or unsupported behavior is required.
