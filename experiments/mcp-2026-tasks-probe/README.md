...
**Deferred future experiment.** Do not run or wire this into normal Shellby MCP development unless the MCP 2026/Tasks roadmap item is intentionally resumed.

This directory is a disposable compatibility test. It does not import or modify Shellby MCP's normal `/mcp` server.
...
The temporary server defaults to Shellby MCP's local port so the existing ngrok URL can be reused. Stop only the normal MCP process; leave the ngrok PM2 process running.

```bash
cd /path/to/shellby-mcp
npx pm2 stop shellby-mcp
...
After testing, stop the temporary server with `Ctrl-C` and restore normal Shellby MCP with `npm run restart`.
