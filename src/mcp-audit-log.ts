...
export class McpAuditLogger {
	constructor(private readonly filePath: string, private readonly now: () => Date = () => new Date(), private readonly clock: () => number = () => Date.now()) {}
	...
	startToolCalls(payload: unknown): McpAuditCall[] {
		...
		return requests.flatMap((request) => {
			...
			this.append(
				formatCallEntry(
					compactTimestamp(this.now()),
					parsed.name,
					parsed.arguments,
					characterCount(serializedArguments),
				),
			);
			...
			return [
				{
					...
					finish: ({ httpStatus, state, outputChars }) => {
						...
						this.append(`${compactTimestamp(this.now())}\tRESULT\t${parsed.name}\tchars=${outputChars}\tduration_ms=${durationMs}\thttp_status=${httpStatus}\tstate=${state}\n\n`);
					},
				},
			];
		});
