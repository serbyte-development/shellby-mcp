...
export class ChatGptConversationTracker {
  ...
  findFinalResponse(query: FinalResponseQuery): TrackedConversationNode | undefined {
    const newNodes = [...this.messages.values()].filter(
      (node) => !query.baselineIds.has(node.id),
    );
...
export class ChatGptSubagentModule {
  ...
  constructor(private readonly options: ChatGptSubagentOptions) {
    ...
    this.minInterTurnDelayMs = nonNegativeInteger(
  ...
  private async createAgent(
    ...
    try {
      ...
      await waitForPromise(
        page.goto(this.chatGptUrl, { waitUntil: "domcontentloaded" }),
        signal,
      );
      throwIfAborted(signal);
  ...
  private getExistingAgent(agentId: string): BrowserAgentState {
    ...
    if (!state) {
      throw new ChatGptSubagentError(
        "UNKNOWN_AGENT",
        `Unknown ChatGPT subagent: ${agentId}`,
      );
    }
    return state;
  }
  ...
  private async ensureActivePage(
    ...
    try {
      await waitForPromise(
        page.goto(state.conversationUrl, { waitUntil: "domcontentloaded" }),
        signal,
      );
      throwIfAborted(signal);
      await assertAuthenticated(page);
      ...
      await page
        ...
        .first()
        .waitFor({ state: "attached", timeout: Math.min(this.timeoutMs, 10_000) });
  ...
  private async waitForResponse(
    ...
    while (true) {
      ...
      if (domFinal?.text) {
        ...
        const now = Date.now();
        if (
          stableCandidate?.key === domFinal.key &&
          stableCandidate.text === domFinal.text
        ) {
  ...
  private async waitForInterTurn(
    state: BrowserAgentState,
    signal?: AbortSignal,
  ): Promise<void> {
    if (state.lastCompletedAt === undefined) return;
    const remaining =
      state.lastCompletedAt + this.minInterTurnDelayMs - Date.now();
...
export function extractConversationMessages(
  ...
  const nodes = new Map(
    extractConversationNodes(payload).map((node) => [node.id, node]),
  );
...
function findNewestMatchingUserNode(
  nodes: readonly TrackedConversationNode[],
  prompt?: string,
): TrackedConversationNode | undefined {
  const users = nodes.filter((node) => node.message.role === "user");
  if (users.length === 0) return undefined;
  const normalizedPrompt = prompt?.trim();
  const exact = normalizedPrompt
    ? users.filter((node) => node.message.text.trim() === normalizedPrompt)
    : [];
  const candidates = exact.length > 0 ? exact : users;
  return [...candidates].sort(
    (left, right) => (right.message.createTime ?? 0) - (left.message.createTime ?? 0),
  )[0];
}
...
function scoreFinalCandidate(
  ...
  if (userNode) {
    if (
      userNode.message.turnExchangeId &&
      candidate.message.turnExchangeId === userNode.message.turnExchangeId
    ) {
      return 300;
    }
  ...
  if (
    sentAtSeconds !== undefined &&
    candidate.message.createTime !== undefined &&
    candidate.message.createTime + 1 < sentAtSeconds
  ) {
    return -1;
  }
  return 10;
...
async function findComposer(
  ...
  throw new ChatGptSubagentError(
    "CHATGPT_UI_CHANGED",
    `Could not find the ChatGPT composer within ${timeoutMs} ms.`,
  );
}
...
async function findNewDomAssistantMessage(
  page: Page,
...
async function loadConversationPayload(
  page: Page,
  conversationId: string,
  timeoutMs: number,
): Promise<unknown> {
...
function isExpectedAgentPage(state: BrowserAgentState): boolean {
  ...
  if (state.conversationUrl) {
    ...
    return expectedConversationId
      ...
      : currentUrl === state.conversationUrl;
  }
