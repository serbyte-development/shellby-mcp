...
export class ShellSessionManager {
  ...
  getOrCreate(shellId = DEFAULT_SHELL_ID): PersistentShellSession {
    ...
    if (this.closingShells.has(shellId)) {
      throw new ShellSessionError(
        "busy",
        `Shell ${JSON.stringify(shellId)} is closing. Retry after it has closed.`,
      );
    }
  ...
  getExisting(shellId = DEFAULT_SHELL_ID): PersistentShellSession {
    ...
    if (this.closingShells.has(shellId)) {
      throw new ShellSessionError(
        "busy",
        `Shell ${JSON.stringify(shellId)} is closing.`,
      );
    }
    const existing = this.sessions.get(shellId);
    if (!existing) {
  ...
  async closeShell(shellId: string): Promise<void> {
    ...
    if (!shell) {
      throw new ShellSessionError(
        "request_not_found",
        `No shell exists for shell_id ${JSON.stringify(shellId)}.`,
      );
    }
  ...
  private assertOpen(): void {
    if (this.closed) {
      throw new ShellSessionError(
        "closed",
        "The shell session manager is closed.",
      );
    }
  }
...
function validateShellId(shellId: string): void {
  if (shellId.length === 0 || shellId.length > MAX_SHELL_ID_LENGTH) {
    throw new ShellSessionError(
      "invalid_command",
      `shell_id must contain between 1 and ${MAX_SHELL_ID_LENGTH} characters.`,
    );
  }
}
...
function cleanupInterval(idleTimeoutMs: number): number {
  return Math.max(
    1_000,
    Math.min(DEFAULT_CLEANUP_INTERVAL_MS, Math.floor(idleTimeoutMs / 2)),
  );
}
