...
test("finds Peekaboo through PATH and accepts an explicit executable", async (t) => {
  ...
  assert.deepEqual(startEvents(await readLog(pathLog))[0]?.args, [
    "list",
    "apps",
    "--json",
  ]);
  assert.deepEqual(startEvents(await readLog(explicitLog))[0]?.args, [
    "list",
    "screens",
    "--json",
  ]);
...
test("bounds oversized stdout and stderr", async (t) => {
  const stdoutClient = fakeClient(
    { FAKE_PEEKABOO_STDOUT_BYTES: "4096" },
    { maxOutputBytes: 256 },
  );
  ...
  const stderrClient = fakeClient(
    { FAKE_PEEKABOO_STDERR_BYTES: "4096" },
    { maxOutputBytes: 256 },
  );
...
test("removes observation artifacts after a Peekaboo error", async (t) => {
  ...
  const error = await peekabooRejection(
    client.observe(["--app", "Finder"], { annotate: false }),
  );
  assert.equal(error.code, "FAKE_COMMAND_FAILED");
