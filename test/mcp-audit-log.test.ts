...
test("formats shell commands as readable blocks and records completion metadata", async (t) => {
  ...
  let clock = 1_000;
  const logger = new McpAuditLogger(file, () => timestamp, () => clock);
  const args = {
    shell_id: "api-audit",
    request_id: "scan-1",
...
test("counts only the JSON-RPC tool result or error payload", () => {
  ...
  assert.equal(
    counts.get("1"),
    characterCount(JSON.stringify({ structuredContent: { response: "hello" } })),
  );
  assert.equal(
    counts.get("two"),
    characterCount(JSON.stringify({ code: -32000, message: "failed" })),
  );
});
...
test("ignores non-tool MCP requests", async (t) => {
  ...
  assert.deepEqual(
    logger.startToolCalls({ jsonrpc: "2.0", id: 1, method: "tools/list" }),
    [],
  );
