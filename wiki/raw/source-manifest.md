...
- The README remains useful for shell/workspace mechanics but is stale for remote authentication, ngrok trust enforcement, PM2 command names, and the MCP v2 package/transport boundary. Maintained wiki pages and current code outrank it for those contracts (`src/auth.ts`, `src/http-server.ts`, `package.json`, `ngrok-traffic-policy.yml`).
...
- Change tripwire: current implementation through commit `55ad62e` (`Upgrade MCP SDK to v2`).
