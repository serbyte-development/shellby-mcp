...
- **Shared-secret arbitrary execution:** every `/mcp` request requires the configured bearer token, but any holder receives the full tool surface and local-user permissions. The token provides no per-tool authorization, expiry, revocation list, or caller identity (`src/http-server.ts`, `src/index.ts`).
- **Caller-selected state boundaries:** named shells isolate runtime state, but authenticated callers are not assigned ownership. Any token holder that knows or guesses another `shell_id` can access or reset that shell, and all shells retain the same operating-system permissions (`src/mcp-server.ts`, `src/shell-session-manager.ts`).
