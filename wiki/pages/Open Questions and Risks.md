...
- **Child-process resource use is not sandboxed:** live named shells are capped at 16, non-default idle shells hibernate, cached logical state expires, and parallel batches add a process-wide four-child ceiling plus a 10-minute per-child timeout. Any active command or child can still consume arbitrary CPU or memory under the local user account, and ordinary persistent-shell commands/background processes remain intentionally untimed until reset/close/hibernation attempts process-group cleanup (`src/tools/shell/session-manager.ts`, `src/tools/shell/session.ts`, `src/tools/shell/parallel-runner.ts`).
