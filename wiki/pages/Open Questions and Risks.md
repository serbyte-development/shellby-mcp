...
- **Peekaboo and permission drift:** the eleven Computer Use schemas are stable at server startup and the default Peekaboo CLI version is pinned by npm, but its JSON fields, daemon/Bridge selection, Screen Recording, Accessibility, and Event Synthesizing permissions can still change across dependency upgrades or an `MCP_PEEKABOO_BIN` override. Calls surface the resulting error and are never retried (`package.json`, `src/tools/computer/peekaboo.ts`, `src/tools/computer/computer-tools.ts`).
