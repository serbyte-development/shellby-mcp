...
- **MCP audit logging can disclose values:** `tools/call` inputs are stored without general redaction in the gitignored repository-local `agent-commands.log`, including prompts, command text, URLs, and other tool arguments. `apply_patch` bodies are intentionally omitted, but their character count and non-patch arguments are logged. Completion records contain output character counts and metadata only, not tool output. `MCP_LOG_COMMANDS` separately controls shell command console logging (`src/index.ts`, `src/http-server.ts`, `src/mcp-audit-log.ts`, `src/shell-session.ts`).
