...
1. `src/index.ts` parses configuration, prepares the workspace, and creates a `ShellSessionManager`, shared `PeekabooClient`, shared `ChatGptSubagentModule`, and repository-local `McpAuditLogger`; the HTTP boundary composes the shared `FeedbackStore` and webpage opener when not injected.
