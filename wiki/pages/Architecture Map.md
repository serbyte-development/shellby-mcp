...
| Computer Use tools     | Publish ten focused schemas, validate targets, and normalize compact MCP results           | `src/computer-use-tools.ts`    |
...
6. `computer_observe` reads a temporary PNG, returns a compact actionable-element map, and retains at most 64 snapshot-to-capture-target mappings. Coordinate actions resolve through that mapping before reaching Peekaboo (`src/peekaboo.ts`, `src/computer-use-tools.ts`).
