...
3. Shell handlers resolve `shell_id` through the shared shell manager. `skill_list` and `skill_use` read `<workspace>/skills` directly on each call, so catalog changes require no server rebuild. `chatgpt_subagent` resolves caller-named `agent_id` through the shared process-level subagent module. `apply_patch` bypasses the shell manager and directly spawns the prepared Codex executable. All request-scoped Computer Use handlers share the same process-level `PeekabooClient`.
