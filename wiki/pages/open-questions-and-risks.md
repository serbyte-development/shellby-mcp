...
- **MCP audit logging can disclose values:** `agent-commands.yaml` can contain sensitive tool inputs even though it is gitignored, permission-restricted, and bounded. Treat the whole file as sensitive; see [Audit Logging](./audit-logging.md) and [Secret Handling](./secret-handling.md).
...
- **Peekaboo and permission drift:** focused Computer Use forces local CLI execution with `--no-remote`, removing daemon/Bridge selection from that path. The pinned CLI's JSON fields, permission requirements, behavior under an `MCP_PEEKABOO_BIN` override, and optional cursor-host binary can still drift. Calls surface semantic failures and are not automatically retried (`package.json`, `src/server/http-server.ts`, `src/tools/computer/peekaboo.ts`, `src/tools/computer/cursor-host.ts`).
