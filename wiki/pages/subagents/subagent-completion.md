...
A tracker binds only to the user-message ID captured from the outgoing conversation POST. Composer serialization can wrap URLs in Markdown links and escape backticks/separators, so even whitespace-normalized prompt text is not a stable identity. Streaming and history recovery share the captured ID. The tracker reconstructs assistant deltas and requires nonempty final text, `finished_successfully`, `end_turn=true`, recipient `all`/empty, and explicit stream completion. Tool-call messages cannot complete a turn.
...
3. Accept history only when user-turn count matches the recorded count and a final answer follows the captured user-message ID. Missing identity or an older identical prompt cannot prove completion.
