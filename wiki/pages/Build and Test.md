...
- Adapter tests cover child MCP discovery, transformed-name dispatch, native schema/result forwarding, missing configured tools, interrupted-action non-retry, next-call reconnect, webpage extraction, and cursors (`test/child-mcp.test.ts`, `test/web-fetch.test.ts`). Peekaboo coverage checks real pinned-package discovery, exact `computer_*` names, the compact-description budget, representative native constraints, image compression, and visual-receipt compaction without performing a GUI action (`test/peekaboo-mcp.test.ts`). HTTP integration additionally verifies child metadata merging plus native image, structured, and error passthrough for an untransformed fixture provider (`test/integrations/computer.ts`). `apply_patch` wrapper behavior and real-binary partial/move semantics are covered by MCP integration tests; the vendored executable also has a host-architecture smoke test (`test/mcp-integration.test.ts`, `test/apply-patch-vendor.test.ts`, [apply_patch](./tools/apply_patch.md)).
