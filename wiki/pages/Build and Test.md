...
- `test/live/chatgpt-fixture-live.test.ts` is the cheap real-service compatibility layer. Its package script loads `.env` when present. It connects to the authenticated Chrome through Playwright/CDP, uses the production unfocused background-page helper and saved-conversation navigator, captures the current `/backend-api/conversations/<id>` response, and passes its flat `messages` payload through the production parser. It compares the extracted branch byte-for-byte with the frozen fixture, checks raw `content.parts`, the conversation route, and current rendered DOM structure, then closes only the temporary page and CDP connection. It never submits a prompt or generates a ChatGPT turn. The project case runs only when `MCP_CHATGPT_PROJECT_URL` is set. Run it explicitly with `npm run test:live:fixture` while the authenticated dedicated Chrome is already running.
