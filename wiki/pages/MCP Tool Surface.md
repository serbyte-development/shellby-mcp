...
| `shell_run`       | Persistent zsh execution plus concurrent batches of independent commands. Canonical caller contract: [[tools/shell_run]]. |
| `shell_poll`      | Continue running work or retained output from `shell_run`. Canonical caller contract: [[tools/shell_run]]. |
...
Shell usage, batching, polling, result fields, and lifecycle semantics are documented once in [[tools/shell_run]]. Runtime implementation details remain in [[pages/Persistent Shell Runtime]]. Web pagination/capture semantics remain separate in the web-tool documentation (`src/tokenizer.ts`, `src/tools/shell/`, `src/tools/web/`).
...
Shell model-facing result behavior is canonical in [[tools/shell_run]]. The registration boundary only projects that result into structured or compact Markdown form (`src/tools/shell/shell-tools.ts`, `src/server/tool-registration-boundary.ts`, `src/server/tool-output.ts`).
