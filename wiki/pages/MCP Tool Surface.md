...
`feedback_submit` is write-only and non-idempotent. It accepts a feedback type, short summary, details, and optional related tool; the shared `FeedbackStore` generates `id` and `created_at` and serializes JSONL appends to `feedback/agent-feedback.jsonl`. The inbox is raw agent feedback for later human triage rather than maintained wiki knowledge (`src/feedback.ts`, `test/mcp-integration.test.ts`).
