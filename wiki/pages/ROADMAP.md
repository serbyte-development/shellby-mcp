...
- [ ] Add global tool-output mode configuration at MCP registration.
...
- [ ] Standardize MCP tool `in / out` token logging after final response transformation.
  ...
  - Keep this simple. If reliable accounting for `subagent_start`, `subagent_poll`, or any other tool requires special-case complexity, omit token logging for that tool rather than complicating the architecture.
